...
if __name__ == '__main__':
    # 加载模型
    # model = YOLO()  # 不使用预训练权重训练
    model = YOLO("E:/pycharm.2022.3.2/Workplace/YOLO11_CLASS/Mine-detection/yolo_model/improvement_yolov11/yolo11.yaml")# 使用预训练权重训练?
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data="E:/pycharm.2022.3.2/Workplace/YOLO11_CLASS/Mine-detection/data/LEVRDataset/data.yaml",
        ...
        project='LEVR',  # (str, optional) 项目名称
        name='YOLO11n',  # (str, optional) 实验名称，结果保存在'project/name'目录下
